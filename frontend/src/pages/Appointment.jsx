                ...
                <div
                    ...
                    <div>
                        <p className={"flex items-center gap-1 text-sm font-medium text-gray-900 mt-3"}>About <img
                            className={"w-3"} src={assets.info_icon} alt=""/></p>
                        <p className={"text-sm text-gray-500 max-w-[700px] mt-1"}>{docInfo.about}</p>
                    </div>
                    <p className={"text-gray-500 font-medium mt-4"}>Appointment fee: <span
                        className={"text-gray-600"}>{currencySymbol}{docInfo.fees}</span></p>
                </div>
            </div>
            {/* ------ Booking slots ------- */}
                    ...
                    {docSlots.length && docSlots[slotIndex].map((item, index) => (
                        <p onClick={() => setSlotTime(item.time)} key={index}
                           className={`text-sm font-light  flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ?
                               "bg-primary text-white" : "text-gray-400 border border-gray-300"}`}>{item.time.toLowerCase()}
                        </p>
                    ))}
                </div>
                <button className={"bg-primary text-white text-sm font-light px-14 py-3 rounded-full my-6"}>Book an
                    appointment
                </button>
            </div>
